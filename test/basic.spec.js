...
describe("Math.round()", () => {
  it("소숫점 이하 값이 0.5 이상이면 올림 값을 반환한다", () => {
    ...
    expect(Math.round(0.4)).not.toBe(1);
  });

  it("소숫점 이하 값이 0.5 미만이면 내림 값을 반환한다", () => {
    expect(Math.round(0.4)).toBe(0);
    expect(Math.round(0.5)).not.toBe(0);
  });
});
